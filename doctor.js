...
io.sockets.on("connection", function (socket) {
	...
	socket.on("joinserver", function(name, symptoms, device) {
		...
		if (exists) {//provide unique username:
			...
			people[socket.id] = {"name" : name, "symptoms": symptoms, "owns" : ownerRoomID, "inroom": inRoomID, "device": device};
	...
	socket.on("doctorjoinserver", function(drs, speciality, device) {
		...
		if (exists) {//provide unique username:
			...
			doctors[socket.id] = {"drs" : drs, "speciality": speciality, "owns" : ownerRoomID, "inroom": inRoomID, "device": device};
			socket.emit("update", "You have connected to the server.");
...
/*
	...
	socket.on("joinserver", function(name, speciality, device) {
		var exists = false;
		var ownerRoomID = inRoomID = null;
		...
		if (exists) {//provide unique username:
			...
			people[socket.id] = {"name" : name, "speciality": speciality, "owns" : ownerRoomID, "inroom": inRoomID, "device": device};
			socket.emit("update", "You have connected to the server.");
			io.sockets.emit("update", people[socket.id].name + " is online.")
			sizePeople = _.size(people);
			sizeRooms = _.size(rooms);
